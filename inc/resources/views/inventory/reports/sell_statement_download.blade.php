    ...
    <main>
      <table>
        ...
        <tbody>
          @php($sl=0)
          @php($balance=0)
          @foreach ($SellStatments as $sell)
          <tr>
              <td class="service">{{ ++$sl }}</td>
              <td class="desc">{{ $sell->inv_pro_inv_invoice_no }}</td>
              <td class="unit">{{ $sell->getCustomerInfo['inv_cus_name'] }}</td>
              <td class="qty">{{ $sell->inv_pro_inv_tran_desc }}</td>
              <td class="total">{{ App\Inv_product_inventory::getTotalDebit($sell->inv_pro_inv_invoice_no) }}</td>
          </tr>
