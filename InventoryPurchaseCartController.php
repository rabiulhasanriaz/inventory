...
class InventoryPurchaseCartController extends Controller
{
    public function addToCart(Request $request)
    {
        ...
        if(!empty($product)) {
            $rows  = Cart::instance('purchase')->content();
            $row = $rows->where('id', $request->pro_id)->first();
            if(!empty($row)) {
                $rowId = $row->rowId;
                Cart::instance('purchase')->update($rowId,['qty' => $request->pro_qty, 'price' => $request->pro_price, 'expire_date' => $request->exp_date]);
            } else {
    ...
    public function addToCartWarrentyProduct(Request $request)
    {
        ...
        if(!empty($product)) {
            ...
            $row = $rows->where('id', $request->pro_id)->first();
            if(!empty($row)) {
                $product_sl_no = $row->options->pro_sl_no;
                $rowId = $row->rowId;
                Cart::instance('purchase')->update($rowId,['price' => $request->pro_price, 'expire_date' => $request->exp_date]);
            } else {
                Cart::instance('purchase')->add([
                    'id' => $product->inv_pro_det_id, 
                    'name' => $product->inv_pro_det_pro_name, 
    ...
    public function addWarrentyProductSlNo(Request $request)
    {
        ...
        if(!empty($row)) {
            ...
            $pre_sl_no = $row->options->pro_sl_no;
            ...
            Cart::instance('purchase')->update($row->rowId, ['qty' => $content_quantity,'options' => ['pro_sl_no' => $pro_all_sl_no]]);

            $all_content = Cart::instance('purchase')->content();
    ...
    public function removeWarrentyProductSlNo(Request $request)
    {
        ...
        // dd($row);
        if(!empty($row)) {
            $pro_all_sl_no = $row->options->pro_sl_no;
    ...
    public function updatecart(Request $request){
        $rows  = Cart::instance('purchase')->content();
        $rowId = $rows->where('id', $request->content_id)->first()->rowId;
        Cart::instance('purchase')->update($rowId,['qty' => $request->pro_qty]);
    }


    public function getCartContent()
    {
        $cart_content = Cart::instance('purchase')->content();
        ...
        try {
            ...
            foreach ($cart_content as $content) {
                ...
                if($check_product->inv_pro_det_pro_warranty != 0) {
                    $request_sl_no_s = $content->options->pro_sl_no;
                }
